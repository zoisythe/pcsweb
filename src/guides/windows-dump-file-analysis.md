...
将你找到的转储文件(优先级为 `Minidump` ≈ `MEMORY.DMP` > `WATCHDOG-***.dmp` )从对应的路径中复制到其他文件夹中(比如下载文件夹、桌面或者 D 盘)，防止后续导入文件时出现权限问题。然后打开 `WinDbg` (全英文界面是正常的，不必担心)，依次选择 `File` - `Open dump file` - `Browse`，然后选中你刚才复制出来的转储文件，点击 `Open` 然后耐心等待软件加载(期间会下载一些文件，请保持网络连接，~~坐和放宽~~)。当你看到如下图中框出的部分不是 `BUSY` 时，输入 `!analyze -v` 然后运行，等待分析完毕。当框中文字不再是 `BUSY` 时，说明分析已经完成，可以开始接下来的操作

![WinDbg 界面](./assets/dump-file/windbg-interface.png)

 ::: warning
 转储分析输出包含你的 Windows 用户名、文件路径、设备信息及其他个人隐私内容。发送给 AI 前请先检查并按需删除或替换这些信息(比如将 `C:\Users\12345\...` 替换为 `C:\Users\***\...` )；不要直接上传完整的 `.dmp` 文件，也不要提交包含敏感信息的截图。

 在你的电脑浏览器上打开一个 AI (Deepseek, 千问，豆包……随便什么都行)(推荐用临时对话或及时删除)，然后复制从输入 `!analyze -v` 开始的输出，脱敏后粘贴给 AI ，然后让它帮你分析。
 :::
...
当你的电脑多次以类似的理由蓝屏且按照 AI 的指示无法解决时，请向官方售后寻求帮助。
